...
this script is meant to plot the observations in a specific time slot in order to compare the McSnow simulations to that. This plots (pol-) moments and (pol-)spectra
    ...
    example: plot spectra in the time range given by -st (start time) and -et (end time)
        python3 plot_obs.py -d 20181110 -st 05:40 -et 05:50
'''
...
import argparse
parser =  argparse.ArgumentParser(description='plot spectra and timeseries')
...
parser.add_argument('-nmn','--noMieNotch', nargs=1,help="dont plot the mie-notch (takes time)",default=["1"])
...
if plot_timeseries:
    ...
    ############################
    plotRout.plotProfilesAndSpectraObs(LDR,LV2,LV0,Peaks,Edges,'plots/spectra/',noMieNotch=noMieNotch)
